  ...
  const handleSelect = (roleId: string, memberId: string) => {
    ...
    mutate(payload, {
      onSuccess: () => {
        ...
        toast({
          title: "Success",
          description: "Members Role changed successfully",
          variant: "success",
        });
      },
      onError: (error) => {
                                ...
                                (role) =>
                                  role.name !== "OWNER" && (
                                    <CommandItem
                                      key={role._id}
                                      disabled={isPending}
                                      className="disabled:pointer-events-none gap-1 mb-1 cursor-pointer flex flex-col items-start px-4 py-2"
                                      onSelect={() => {
                                        handleSelect(
                                          role._id,
                                          member.userId?._id
                                    ...
                                    >
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        ...
                                        {role.name === "MEMBER" &&
                                          "Can view and edit only tasks they created."}
                                      </p>
                                    </CommandItem>
                                  )
                              )}
